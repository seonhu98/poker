...
namespace pokerProject
{
    public static class Ractangle
    {
        ...
        public static OpenCvSharp.Point[] Square(Mat src)
        {
            ...
            {
                Cv2.GaussianBlur(split[channel], blur, new OpenCvSharp.Size(5, 5), 5);
                for (int i = 0; i < N; i++)
                {
        ...
        public static string OCR(Mat src)
        {
            ...
            var bitmap =  Tesseract.Pix.LoadFromMemory(src.ToBytes());
            var ocr = new TesseractEngine(@"C:\Users\lms\source\repos\pokerProject\tessdata\", "eng", EngineMode.LstmOnly);
            ocr.SetVariable("tessedit_char_whitelist",",AKQJ0123456789");
